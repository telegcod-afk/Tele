...
# Telegram may return this when trying to edit
# a message that already has the same content.
if (
    isinstance(exception, TelegramBadRequest)
    and "message is not modified" in str(exception).lower()
):
    logging.debug(
        "Telegram ignored duplicate message edit: %s",
        exception
    )
    return True

# Keep normal aiogram error handling for other errors.
return False
...
Loading middleware is registered first so callback ACK

can be sent before maintenance/ban/rate-limit/database work.
